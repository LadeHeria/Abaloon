...
void generer_places(cases tableau[60], int decalage_x, int decalage_y, int debutx, int debuty){
	...
	for(int i=1; i<6; i++){
		tableau[55-i].x=debutx+i*decalage_x;
	...
	for(int i=1; i<7; i++){
		tableau[49-i].x=debutx+i*decalage_x;
	...
	for(int i=1; i<8; i++){
		tableau[42-i].x=debutx+i*decalage_x;
	...
	//ligne 7
	for(int i=0; i<8; i++){
		tableau[18-i].x=tableau[49-i].x;
		tableau[18-i].y=tableau[49-i].y+4*decalage_y;
	}
	//ligne 8
	for(int i=0; i<7; i++){
		tableau[11-i].x=tableau[55-i].x;
		tableau[11-i].y=tableau[55-i].y+6*decalage_y;
	}
	//ligne 9
	for(int i=0; i<6; i++){
		tableau[5-i].x=tableau[60-i].x;
		tableau[5-i].y=tableau[60-i].y+8*decalage_y;
	}
}
